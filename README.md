...
|---------|-----------|---------|----------|----------|-----------|
| 1 | 2024-01-01T10:00:00 | 1-1 | 10:00:01 | 10:00:15 | 14000 |

**저장 위치**: 피험자별 고정 위치 (피험자 N → (N-1)*4+2 ~ (N-1)*4+5 행)
...
3. **시작 버튼 클릭**: 첫 번째 질문이 입력창에 자동 설정
4. **질문 전송**: 화살표 버튼으로 질문 전송 (시간 측정 시작)
5. **응답 대기**: "잠시만 기다려 주세요" 메시지 표시
6. **중지 버튼 클릭**: 정지 아이콘 클릭으로 대기 중단 (시간 측정 종료)
7. **다음 질의**: 4개 질문 반복 진행
8. **실험 종료**: 마지막 질문 완료 시 고정 위치에 결과 저장
...
- **고정 위치 저장**: 동시 사용자 간 데이터 충돌 방지를 위한 전용 행 할당
